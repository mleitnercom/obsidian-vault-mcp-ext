"""Tests for the create-only note tool, ported with the tool from the fork (v0.11.0).
...
import obsidian_vault_mcp.config as host_config
from obsidian_vault_mcp import write_events

from obsidian_vault_mcp_ext.createnote import _config as config
from obsidian_vault_mcp_ext.createnote.tools import vault_create_note
...
def vault(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(host_config, "VAULT_PATH", root)
...
# --------------------------------------------------------------------------
# Unconfigured: see test_createnote_generic.py
# --------------------------------------------------------------------------
...
def test_creation_is_audited_and_announced_once(vault, tmp_path, monkeypatch):
    ...
    try:
        result = json.loads(vault_create_note(PATH, _note()))
    finally:
        write_events._write_listeners.clear()

    assert result.get("created") is True, result
    assert events == [("created", [PATH])]
    records = [json.loads(line) for line in log.read_text(encoding="utf-8").splitlines()]
    assert [(r["operation"], r["operation_status"]) for r in records] == [("vault_create_note", "success")]


def test_a_refused_creation_is_audited_as_an_error_and_announced_to_nobody(vault, tmp_path, monkeypatch):
    log = tmp_path / "audit.jsonl"
    monkeypatch.setattr(host_config, "VAULT_AUDIT_LOG_PATH", str(log))
    (vault / PATH).write_text("schon da", encoding="utf-8")
    write_events._write_listeners.clear()
    events = []
    write_events.register_write_listener(lambda op, paths: events.append((op, paths)))
    try:
    ...
    assert result["error_code"] == "note_exists", result
    assert events == []
    records = [json.loads(line) for line in log.read_text(encoding="utf-8").splitlines()]
    assert [(r["operation"], r["operation_status"]) for r in records] == [("vault_create_note", "error")]


def test_validation_refusals_leave_no_audit_record(vault, tmp_path, monkeypatch):
    """Policy refusals happen before anything is written; only write attempts are recorded."""
    log = tmp_path / "audit.jsonl"
    monkeypatch.setattr(host_config, "VAULT_AUDIT_LOG_PATH", str(log))

    result = json.loads(vault_create_note("anderswo/x.md", _note()))
