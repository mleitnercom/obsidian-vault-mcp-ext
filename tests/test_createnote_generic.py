...
Ported from the fork's tests/test_create_note_generic.py (v0.15.0). All calls go through
the tool as CreateNoteExtension registers it, in a temp vault. Unlike the fork, the host
passes content through unchanged, so notes keep their trailing newline.
"""
...
from obsidian_vault_mcp_ext.createnote import CreateNoteExtension
from obsidian_vault_mcp_ext.createnote import _config as config

_server = FastMCP("createnote-generic")
CreateNoteExtension().register_tools(_server)
...
def create(path: str, content: str) -> dict:
    result = asyncio.run(_server.call_tool("vault_create_note", {"path": path, "content": content}))
...
def test_the_answer_on_a_taken_name_steers_a_model_away_from_vault_write():
    ...
    tool = next(t for t in asyncio.run(_server.list_tools()) if t.name == "vault_create_note")
...
def test_a_long_note_is_accepted_up_to_the_general_limit(vault, monkeypatch):
    ...
    monkeypatch.setattr(host_config, "MAX_CONTENT_SIZE", 50_000)
