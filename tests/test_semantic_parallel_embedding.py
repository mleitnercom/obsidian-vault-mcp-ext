...
Runs through the `vault-mcp-ext-semantic` CLI with real FAISS. Most tests use a recording embedder;
...
from obsidian_vault_mcp_ext.semantic import _config as config, cli as semantic_cli
from obsidian_vault_mcp_ext.semantic.engine import SemanticSearchEngine
from obsidian_vault_mcp_ext.semantic.models import Chunk
...
def env(tmp_path, monkeypatch, capsys):
    ...
    def cli(cache, *args, parallel=0):
        ...
        monkeypatch.setattr(config, "SEMANTIC_CACHE_PATH", tmp_path / cache, raising=False)
        before = len(embedders)
        assert semantic_cli.cli_main(list(args)) == 0
...
def test_a_stream_of_the_wrong_length_fails_and_keeps_the_cache(env, drift, monkeypatch, capsys):
    ...
    monkeypatch.setattr(config, "SEMANTIC_EMBED_PARALLEL", 4, raising=False)
    monkeypatch.setattr(config, "SEMANTIC_CACHE_PATH", env.root / "B", raising=False)
    assert semantic_cli.cli_main(["reindex", "--mode", "full"]) == 1
    assert "vectors for" in capsys.readouterr().out
...
def test_the_real_model_in_real_worker_processes(tmp_path, monkeypatch, capsys, caplog):
    ...
    monkeypatch.setattr(config, "SEMANTIC_EMBED_BATCH_SIZE", 4)
    caplog.set_level(logging.INFO, logger="obsidian_vault_mcp_ext.semantic.engine")

    def run(cache, parallel):
        monkeypatch.setattr(config, "SEMANTIC_EMBED_PARALLEL", parallel, raising=False)
        monkeypatch.setattr(config, "SEMANTIC_CACHE_PATH", tmp_path / cache, raising=False)
        caplog.clear()
        assert semantic_cli.cli_main(["reindex", "--mode", "full"]) == 0
        capsys.readouterr()
        log = caplog.text
        chunks = json.loads((tmp_path / cache / "chunks.json").read_text(encoding="utf-8"))
        index = faiss.read_index(str(tmp_path / cache / "faiss.index"))
