...
Until 0.4.0 `--mode incremental` found the changed files correctly and then re-embedded
...
Everything runs through the `vault-mcp-ext-semantic` CLI, as the nightly job does, with real
...
def semantic(tmp_path, monkeypatch):
    ...
    def cli(cache, *args):
        """One `vault-mcp-ext-semantic` run: a fresh engine that loads from `cache`."""
        # _config derives the cache path lazily; a real attribute overrides it.
...
def test_the_updated_cache_serves_searches(semantic):
    ...
    # This CLI has no search command: search as the server does, with a fresh engine on the cache.
    found = SemanticSearchEngine().search("Quittenkompott", search_mode="keyword")
