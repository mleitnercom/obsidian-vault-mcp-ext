...
Ported from the fork's retrieval/engine.py onto upstream-public APIs only.
Adaptations vs. the fork source:

1. ``from .. import config`` -> ``from . import _config as config`` (self-contained config).
2. The fork imported ``is_vault_path_allowed`` from ``..vault`` (a fork-only scope-policy
   helper that does NOT exist upstream). It is replaced by ``_resolves_in_vault`` below:
   a path is allowed iff it resolves inside the vault via the host's public
   ``resolve_vault_path`` (ValueError -> not allowed). The fork's INCLUDED/EXCLUDED_ROOTS
   scope policy is intentionally out of scope; all in-vault paths are treated as allowed.
3. faiss / numpy / fastembed / sentence_transformers / rank_bm25 stay lazily imported
   inside methods (never hoisted to module top), so importing this module needs none of
   the heavy [semantic] deps.
"""
...
def _resolves_in_vault(abs_path: Path) -> bool:
    """Extension-local replacement for the fork's ``is_vault_path_allowed``.

    A path is allowed iff its vault-relative form resolves back inside the vault
    via the host's public ``resolve_vault_path`` (which rejects dotfiles and any
    path escaping the vault root). On any ValueError the path is treated as not
    allowed (fail closed).
    """
    try:
        rel = abs_path.relative_to(config.VAULT_PATH)
    except ValueError:
        return False
    try:
        resolve_vault_path(rel.as_posix())
    except ValueError:
        return False
    return True
...
class SemanticSearchEngine:
    ...
    def search(
        ...
        try:
            ...
            # Dependencies missing or backend unbuildable: fail soft with the reason
            # the engine recorded rather than propagating a crash to the tool layer.
            return {"error": self._unavailable_reason or f"Semantic search is unavailable: {exc}"}
        with self._lock:
            if not self.enabled:
    ...
    def _incremental_reindex_unlocked(self, updates: dict[str, str]) -> dict:
        ...
        # embedded. Until 0.4.0 every incremental run re-embedded the whole vault.
    ...
    def _is_indexable_path(rel_path: str) -> bool:
        ...
        Adaptation of the fork's check: the fork called ``is_vault_path_allowed``
        (its scope-policy helper). Here a path is indexable iff it is markdown, has
        no excluded directory component, and resolves inside the vault.
        """
