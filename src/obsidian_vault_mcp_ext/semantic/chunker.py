...
Ported from the fork's retrieval/chunker.py. Adaptation: ``from .. import config``
is replaced with ``from . import _config as config`` (self-contained config).
``frontmatter`` (python-frontmatter) is a host dependency, so it stays a normal
top-level import; the heavy embedding deps (faiss/numpy/fastembed) live only in
engine.py and are imported lazily there.
"""
