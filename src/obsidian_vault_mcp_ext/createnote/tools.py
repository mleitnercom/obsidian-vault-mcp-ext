...
def vault_create_note(path: str, content: str) -> str:
    ...
    try:
        ...
        # Audited and announced as "created" only when the note exists and reads back;
        # a refusal inside is recorded as an error and announced to nobody.
        with mutation("vault_create_note", path) as m:
            try:
            ...
            # Report success only for bytes that are actually readable back. This also
            # exercises the host's hardlink guard in read_file: the exclusive create goes
            # through os.link, and a temp link left behind would make the new note
            # unreadable to every other tool.
